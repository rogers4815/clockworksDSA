...
public class UserSubmission {
	...
	private int sendWOPPing(){
        try{
        	URL serverURL = new URL("http://" + server + ":" + port + "/resultassemblyhandler"); //throws MalformedURLException
    		...
    		serverConnection.addRequestProperty("Environment-Id", Integer.toString(environmentID));
            ...
            if(response == 200){
                ...
                BufferedOutputStream file = new BufferedOutputStream(new FileOutputStream("results.dsa"));
                file.write(result);
                file.flush();
                file.close();
            }
	...
	private int sendSimulation(String filepath){
		try{
			URL serverURL = new URL("http://" + server + ":" + port + "/environmenthandler"); //throws MalformedURLException
			...
			DataOutputStream output = new DataOutputStream(sendConnection.getOutputStream());
			...
			if(sendConnection.getResponseCode() == 200){
				environmentID = Integer.parseInt(sendConnection.getContent().toString());
			}
