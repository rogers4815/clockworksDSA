...
/**
 * Look for the interpreter, and ask to install it if not found
 */
public class DialogActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String interpreter = FeaturedInterpreters
				.getInterpreterNameForScript(scriptName);
		...
		if (resolveInfos != null && resolveInfos.size() == 1) {
			...
			activityIntent.setComponent(new ComponentName(info.packageName,
					info.name));
		...
		dialog.setMessage(String
				.format("Do you want to download and install APK for %s ?",
						interpreter));
